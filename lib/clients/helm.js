...
class Helm {
  ...
  async installDeps(index=0) {
    ...
    await this._createNamespace(constants.defaultNamespace);
    await this._createNamespace(constants.defaultNamespace);
  ...
  async install(){
    ...
    for (const n of Array(this.config.nodes).keys()) {
      ...
      try {
        await this._cmd(`upgrade --install ${name} --namespace ${constants.defaultNamespace} -f ${relativeValuesFilePath} --version ${chartVersion} ${chartName}`);
  ...
  async _installPolkadotBaseServices(index) {
    ...
    await this._cmd(`upgrade --install polkadot-base-services --namespace ${constants.defaultNamespace} -f ${relativeValuesFilePath} --version ${baseVersion} w3f/polkadot-base-services`);
  ...
  async _installPolkadotSecrets() {
    ...
    this.files.writeYAML(fullValuesFilePath, values);
    const baseVersion = this.mainConfig.chartVersions.polkadot;
    await this._cmd(`upgrade --install polkadot-secrets --namespace ${constants.defaultNamespace} -f ${relativeValuesFilePath} --version ${baseVersion} w3f/polkadot-secrets`);
  ...
  async _createNamespace(namespace) {
    try {
      await this.kubectl.cmd(`create namespace ${namespace}`);
    } catch(e) {
      console.log(`Could not create namespace: ${e}`);
    }
  }
}
